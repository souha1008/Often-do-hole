...
public class Gimmick_FallBlock_2 : Gimmick_Main
{
    ...
    public MeshRenderer FallBlockMashObject;
    ...
    public override void FixedMove()
    {
        if (ActiveFlag)
        {
            ...
            if (NowFall && NowTime <= 0)
            {
                ...
                SoundManager.Instance.PlaySound("sound_28", 1.0f, 0.3f);
            }
            ...
            if (NowFall)
            {
                ...
                if (ShakeFlag)
                {
                    ...
                    if (NowShakeTime >= Shake.ShakeTime)
                    {
                        ...
                        NowShakeTime = 0.0f;
                        // 普通の落下移動に戻す
                        FallBlockMashObject.transform.localPosition = Vector3.zero;
                    }
                }
                else // 普通の落下移動
                {
    ...
    private void NoActive()
    {
        ...
        FallBlockMashObject.enabled = false;
        NowTime = 0;
        ActiveFlag = false;
    ...
    private void Active()
    {
        ...
        FallBlockMashObject.enabled = true;
        NowTime = 0;
        SizeUpNowTime = 0;
        ActiveFlag = true;
    }
