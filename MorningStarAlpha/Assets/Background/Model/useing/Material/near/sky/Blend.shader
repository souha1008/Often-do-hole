...
Shader "Skybox/blend" {
    Properties{
        _Tint("Tint Color Morning", Color) = (.5, .5, .5, .5)
        _TintEvening("Tint Color Evening", Color) = (.5, .5, .5, .5)
        _TintNight("Tint Color Night", Color) = (.5, .5, .5, .5)
        [Gamma] _Exposure("Exposure Morning", Range(0, 8)) = 1.0
        [Gamma] _ExposureEvening("Exposure Evening", Range(0, 8)) = 1.0
        [Gamma] _ExposureNight("Exposure Night", Range(0, 8)) = 1.0
        _Rotation("Rotation", Range(0, 360)) = 0
        [NoScaleOffset] _MainTex("SphericalA  (HDR)", 2D) = "grey" {}
        [NoScaleOffset] _MainTexB("SphericalB  (HDR)", 2D) = "grey" {}
        ...
        SubShader{
            ...
            Pass {
                ...
                fixed4 frag(v2f i) : SV_Target
                {
                    ...
                    half4 texA = tex2D(_MainTex, tc);
                    half4 texB = tex2D(_MainTexB, tc);
                    half4 tex = lerp(texA, texB, saturate(_value - 1));
                    half3 c = DecodeHDR(tex, _MainTex_HDR);

                    half4 tint_me = lerp(_Tint, _TintEvening, saturate(_value));
                    half4 tint_en = lerp(_TintEvening, _TintNight, saturate(_value - 1));
                    half4 tint = lerp(tint_me, tint_en, saturate(_value));
                    c = c * tint.rgb * unity_ColorSpaceDouble.rgb;

                    half expo_me = lerp(_Exposure, _ExposureEvening, saturate(_value));
                    half expo_en = lerp(_ExposureEvening, _ExposureNight, saturate(_value - 1));
                    half expo = lerp(expo_me, expo_en, saturate(_value));
                    c *= expo;
                    return half4(c, 1);
                }
                ENDCG
